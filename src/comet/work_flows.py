...
class WorkflowRunner(WorkflowBase):
    """Backend to handle Gitflow based development work flows.
    ...
    SUPPORTED_JOB_METHODS = {
        "lookup_commits": {
            "method": "lookup_commits",
            "internal_parameters": [
                "project",
                "source_ref"
            ],
    ...
    def sanity_checks(self):
        source_flows = [
            flow["name"] for flow in self.project_config.config["strategy"]["flows"]
            if self.source_branch in flow["branches"]
        ]
    ...
    def fetch_flow(self, branch: str, flow_type: str):
        flows = []
        for flow in self.project_config.config["strategy"]["flows"]:
            if flow["type"] == flow_type and branch in flow["branches"]:
                flows.append(flow)
        return flows
    ...
    def branch_flow(self):
        assert len([
            flow["name"] for flow in self.project_config.config["strategy"]["flows"]
            if self.source_branch in flow["branches"]
        ]) < 2, f"Multi branch flows found for the source branch [{self.source_branch}]"

        flow = self.fetch_flow(self.source_branch, "branch_flow")
        print(flow)
        ...
        for job in jobs:
            method_to_call = getattr(self, job["job"])
