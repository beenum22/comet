...
class TestGitFlow(unittest.TestCase):
    ...
    def test_prepare_workflow(self, mock_scm, mock_configparser):
        ...
        flow = GitFlow(
            scm_provider=TestBaseConfig.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=TestBaseConfig.TEST_GIT_CONFIG["connection_types"][0],
            username=TestBaseConfig.TEST_GIT_CONFIG["username"],
            password=TestBaseConfig.TEST_GIT_CONFIG["password"],
            ssh_private_key_path=TestBaseConfig.TEST_GIT_CONFIG["ssh_key_path"],
            project_local_path=TestBaseConfig.TEST_REPO_DIRECTORY,
            project_config_path=TestBaseConfig.TEST_GITFLOW_CONFIG_FILE,
            push_changes=False,
            git_notes_state_backend=False
        )

        mock_configparser.assert_called_once_with(
            config_path=f"{TestBaseConfig.TEST_REPO_DIRECTORY}/{TestBaseConfig.TEST_GITFLOW_CONFIG_FILE}",
            skip_state=False
        )
        ...
        mock_scm.assert_called_once_with(
            ...
            repo_local_path=TestBaseConfig.TEST_REPO_DIRECTORY,
            ssh_private_key_path=TestBaseConfig.TEST_GIT_CONFIG["ssh_key_path"],
    ...
    def test_prepare_versioning(
        ...
        gitflow_v1.prepare_versioning()

        for project in TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"]:
    ...
    @patch.object(GitFlow, 'release_candidate_flow', autospec=True)
    @patch.object(GitFlow, 'release_to_stable_flow', autospec=True)
    @patch("src.comet.work_flows.ConfigParser", autospec=True)
    @patch("src.comet.work_flows.Scm", autospec=True)
    @patch("src.comet.work_flows.SemVer", autospec=True)
    def test_release_flow(
        ...
        mock_scm().get_active_branch.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["development_branch"]
        ...
        gitflow_mono_v1.prepare_versioning()
        ...
        mock_stable_release.assert_called_once_with(gitflow_mono_v1, gitflow_mono_v1.source_branch)
        ...
        mock_stable_release.reset_mock()
        mock_release_candidate.reset_mock()
        mock_configparser.return_value.config = TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]
        ...
        gitflow_multi_v1.prepare_versioning()
    ...
    def test_sync_flow(
        ...
        gitflow_mono_v1_with_push.prepare_versioning()
    ...
    def test_branch_flows(
        ...
        logger.info("Executing unit tests for 'GitFlow.branch_flow' method")
        ...
        mock_configparser().get_development_model_options.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]
        mock_scm().get_active_branch.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["stable_branch"]
        gitflow_mono_v1 = GitFlow(
            scm_provider=TestBaseConfig.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=TestBaseConfig.TEST_GIT_CONFIG["connection_types"][0],
            username=TestBaseConfig.TEST_GIT_CONFIG["username"],
        ...
        mock_configparser().get_development_model_options.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]
        mock_scm().get_active_branch.return_value = \
            f'{TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["release_branch_prefix"]}/test'
        gitflow_mono_v1 = GitFlow(
            scm_provider=TestBaseConfig.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=TestBaseConfig.TEST_GIT_CONFIG["connection_types"][0],
            username=TestBaseConfig.TEST_GIT_CONFIG["username"],
    ...
    def test_release_project_version(
        ...
        mock_scm().get_active_branch.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["development_branch"]
        mock_scm().find_new_commits.side_effect = TestGitFlow.side_effect_find_new_commits
        mock_semver().get_version.return_value = TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"]
        mock_semver().get_final_version.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]
        gitflow_mono_v1 = GitFlow(
            scm_provider=TestBaseConfig.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=TestBaseConfig.TEST_GIT_CONFIG["connection_types"][0],
            username=TestBaseConfig.TEST_GIT_CONFIG["username"],
        ...
        self.assertTrue(
            gitflow_mono_v1.release_project_version(
                TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["path"],
                TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["development_branch"]
            )
        )
        ...
        mock_semver().update_version_files.assert_called_once_with(
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"],
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]
        )
    ...
    @patch("src.comet.work_flows.Scm", autospec=True)
    @patch("src.comet.work_flows.SemVer", autospec=True)
    def test_release_to_stable_flow(
            self,
            mock_semver,
            mock_scm,
            mock_configparser,
        ...
        logger.info("Executing unit tests for 'GitFlow.release_to_stable_flow' method")
        ...
        mock_semver().get_final_version.side_effect = [
            project["version"].split("-")[0] for project in TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["projects"]
        ]
        ...
        logger.debug("Testing stable release flow from development branch for multi repo with v1/new "
                     "config format")
        release_projects = \
            gitflow_multi_v1.release_to_stable_flow(TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["development_branch"])

        self.assertCountEqual(
            release_projects,
            [project_dict["path"] for project_dict in TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["projects"]]
        )
        release_project_calls = [call(
            project["path"],
            TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["development_branch"]
        ) for project in TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["projects"]]
        ...
        mock_scm().commit_changes.assert_called_once_with(
            ...
            f"{TestBaseConfig.TEST_REPO_DIRECTORY}/{TestBaseConfig.TEST_GITFLOW_CONFIG_FILE}",
            *release_projects,
            push=True
        )
        mock_scm().merge_branches.assert_called_once_with(
            source_branch=TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["development_branch"],
            destination_branch=TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["stable_branch"]
        )
        mock_scm().add_tag.assert_has_calls(tag_calls)
        mock_scm().push_changes.assert_called_once_with(
            branch=TestBaseConfig.TEST_GITFLOW_CONFIGS["multi"]["v1"]["stable_branch"],
            tags=True
        )
        ...
        logger.debug("Testing stable release flow from an invalid branch for multi repo with v1/new "
                     "config format")
        with self.assertRaises(AssertionError):
            gitflow_multi_v1.release_to_stable_flow("feature/test")
    ...
    def test_create_project_rc(
        ...
        logger.debug("Initializing GitFlow object for mono repo")
        mock_configparser.return_value.config = TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]
        mock_configparser().has_deprecated_versioning_format.return_value = False
        mock_configparser().get_development_model_options.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]
        mock_scm().has_local_branch.side_effect = TestGitFlow.side_effect_release_branch
        mock_scm().has_remote_branch.side_effect = TestGitFlow.side_effect_release_branch
        mock_configparser().get_project_version.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"]
        mock_semver().get_version.return_value = \
            f'{TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]}-rc.1'
        mock_semver().get_final_version.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]
        ...
        mock_scm().has_local_branch.assert_called_with(
            f'{TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]["release_branch_prefix"]}/'
            f'{TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]}'
        )
    ...
    def test_upgrade_stable_branch_project_version(
        ...
        logger.debug("Initializing GitFlow object for mono repo with v1/new config format")
        current_stable_version = TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0]
        new_stable_version = \
            f'{".".join(TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0].split(".")[:-1])}.' \
            f'{int(TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["projects"][0]["version"].split("-")[0].split(".")[-1]) + 1}'
        mock_configparser.return_value.config = TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]
        mock_configparser().has_deprecated_versioning_format.return_value = False
        mock_configparser().get_development_model_options.return_value = \
            TestBaseConfig.TEST_GITFLOW_CONFIGS["mono"]["v1"]["strategy"]["development_model"]["options"]
        ...
        mock_scm().get_commit_message.side_effect = TestGitFlow.side_effect_get_commit_message
