...
class GitFlowTestV0(unittest.TestCase, TestBaseConfig, TestBaseCommitMessages):
    ...
    @patch.object(GitFlow, 'release_candidate_flow')
    @patch.object(GitFlow, 'release_to_stable_flow')
    @patch("src.comet.work_flows.ConfigParser")
    @patch("src.comet.work_flows.Scm")
    @patch("src.comet.work_flows.SemVer")
    def test_release_flow(
        ...
        gitflow_mono_v0.prepare_versioning(reference_version_type=None)

        logger.debug("Testing stable release flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.release_flow(branches=False)
        mock_stable_release.assert_called_once_with(gitflow_mono_v0.source_branch)
        mock_release_candidate.assert_not_called()

        mock_stable_release.reset_mock()
        mock_release_candidate.reset_mock()

        logger.debug("Testing release candidate flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.release_flow(branches=True)
        ...
        logger.debug("Testing release candidate flow for multi repo with v0/deprecated config format")
        with self.assertRaises(AssertionError):
            gitflow_multi_v0.release_flow(branches=True)
    ...
    def test_branch_flow(
        ...
        mock_scm().get_active_branch.return_value = self.TEST_GITFLOW_CONFIGS["mono"]["v0"]["development_branch"]
        gitflow_mono_v0 = GitFlow(
            scm_provider=self.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=self.TEST_GIT_CONFIG["connection_types"][0],
            username=self.TEST_GIT_CONFIG["username"],
        ...
        logger.debug("Testing development branch flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.branch_flow()
        ...
        mock_configparser.return_value.config = self.TEST_GITFLOW_CONFIGS["mono"]["v0"]
        mock_scm().get_active_branch.return_value = self.TEST_GITFLOW_CONFIGS["mono"]["v0"]["stable_branch"]
        gitflow_mono_v0 = GitFlow(
            scm_provider=self.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=self.TEST_GIT_CONFIG["connection_types"][0],
            username=self.TEST_GIT_CONFIG["username"],
        ...
        logger.debug("Testing stable branch flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.branch_flow()
        ...
        mock_scm().get_active_branch.return_value = \
            f'{self.TEST_GITFLOW_CONFIGS["mono"]["v0"]["release_branch_prefix"]}/test'
        gitflow_mono_v0 = GitFlow(
            scm_provider=self.TEST_GIT_CONFIG["scm_providers"][0],
            connection_type=self.TEST_GIT_CONFIG["connection_types"][0],
            username=self.TEST_GIT_CONFIG["username"],
        ...
        logger.debug("Testing release branch flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.branch_flow()
        ...
        logger.debug("Testing default branch flow for mono repo with v0/deprecated config format")
        gitflow_mono_v0.branch_flow()
    ...
    def side_effect_find_new_commits(source_branch: str, destination_branch: str, project_path: str) -> list:
        project_commits = sample(
            list(TestBaseCommitMessages.TEST_DUMMY_COMMITS.values()),
    ...
    def test_release_project_version(
        ...
        mock_scm().find_new_commits.side_effect = GitFlowTestV0.side_effect_find_new_commits
    ...
    def test_release_to_stable_flow(
        ...
        mock_scm().get_active_branch.return_value = self.TEST_GITFLOW_CONFIGS["multi"]["v0"]["development_branch"]
        ...
        logger.debug("Testing stable release flow from development branch for multi repo with v0/deprecated "
                     "config format")
        release_projects = \
            gitflow_multi_v0.release_to_stable_flow(self.TEST_GITFLOW_CONFIGS["multi"]["v0"]["development_branch"])
        ...
        logger.debug("Testing stable release flow from an invalid branch for multi repo with v0/deprecated "
                     "config format")
        with self.assertRaises(AssertionError):
            gitflow_multi_v0.release_to_stable_flow("feature/test")
    ...
    def test_create_project_rc(
        ...
        mock_scm().get_active_branch.return_value = self.TEST_GITFLOW_CONFIGS["mono"]["v0"]["development_branch"]
        mock_scm().has_local_branch.side_effect = GitFlowTestV0.side_effect_release_branch
        mock_scm().has_remote_branch.side_effect = GitFlowTestV0.side_effect_release_branch
...
class GitFlowTestV1(unittest.TestCase, TestBaseConfig, TestBaseCommitMessages):
    ...
    def test_release_to_stable_flow(
        ...
        logger.debug("Testing stable release flow from development branch for multi repo with v1/new "
                     "config format")
        release_projects = \
            gitflow_multi_v1.release_to_stable_flow(self.TEST_GITFLOW_CONFIGS["multi"]["v1"]["development_branch"])
